...
## General-method framing, graphs, and new cross-validation, v0.3.0

The title now describes Flint as a general method for extracting structure from EEG. The method and its distinct readouts are described explicitly; the runnable release still starts from retained participant features, with original EEG-extraction source preserved separately.

`scripts/cross_validate_readouts.py` computes new feature-table cross-validation for three historically chosen EEG scores, two targets, and two calibration models. All 111 participant IDs are aligned explicitly, predictor scaling and calibration are training-only, and all outcomes are observed. The output includes 27,972 predictions, exact folds, 252 run metrics, 24 summaries, and the full run manifest. `scripts/write_cv_report.py` renders the report directly from those summaries.

`scripts/plot_results.py` adds 11 figures in PNG and SVG. The five complete domain heatmaps account for all 1,164 retained phenotype rows. Phenotype curves remain visualizations of historical summaries; new scalar-score cross-validation is a separate computation. No original participant features or archived operator sources were changed.
